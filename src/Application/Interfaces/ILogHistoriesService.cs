...
namespace LotTrace_MES.src.Application.Interfaces
{
    ...
    {
        Task<IEnumerable<LogHistories>> GetLogHistoriesByLotIdAsync(int lotId); // LotId로 검색
        Task<IEnumerable<LogHistories>> GetLogHistoriesByBarcodeAsync(string barcode); // 바코드 조회
